...
export const useStore = create<RFState>((set, get) => ({
  ...
  unaddedFiles: [
    ...
    },
    {
      ...
      fileName: "queries.ts",
      contents: "# Sidebar for dragging files",
    },
    {
  ...
  removeFilesFromCanvas: (nodeIds) => {
    const inCanvas = get().nodes.filter((node) => !nodeIds.includes(node.id));
    const offCanvas = get().nodes.filter((node) => nodeIds.includes(node.id));
    set({
      nodes: inCanvas,
      unaddedFiles: [
        ...get().unaddedFiles,
